...
def fetch_fii_dii() -> dict:
    ...
    if not FORCE_SHEETS and not FORCE_YFINANCE and nse_ok:
        try:
            ...
            # FIX-STREAK: apply consecutive-session bonus from SQLite history
            streak = _get_fii_streak()
            score  = min(38, score + streak["streak_bonus"])  # cap raised to 38 to reflect streak signal
            if streak["streak_label"]:
                label = f"{label} | {streak['streak_label']}"
    ...
    if _sheets_ok():
        ...
        if not df.empty:
            ...
            if fii_col and dii_col:
                ...
                # FIX-STREAK: apply consecutive-session bonus from SQLite history
                streak = _get_fii_streak()
                score  = min(38, score + streak["streak_bonus"])
                if streak["streak_label"]:
                    label = f"{label} | {streak['streak_label']}"
...
# ── FIX-STREAK: FII/DII consecutive-session accumulation tracking ─────────────
def _append_fii_dii_history(fii_data: dict, trade_date: str) -> None:
    """
    FIX-STREAK: Persist today's FII/DII net figures to fii_dii_history table.
    Called once in run() after intelligence fetch.  Rolls table to 30 days.
    """
    fii_net = fii_data.get("fii_net", 0)
    dii_net = fii_data.get("dii_net", 0)
    both    = 1 if (fii_net > 0 and dii_net > 0) else 0
    try:
        with _db_conn(write=True) as con:
            con.execute(
                "INSERT OR REPLACE INTO fii_dii_history "
                "(trade_date, fii_net_cr, dii_net_cr, both_buying) VALUES (?,?,?,?)",
                (trade_date, float(fii_net), float(dii_net), both)
            )
            con.execute(
                "DELETE FROM fii_dii_history WHERE trade_date < date(?, '-30 days')",
                (trade_date,)
            )
        log.info(f"FIX-STREAK: FII/DII history saved for {trade_date} "
                 f"(fii={fii_net:+.0f}Cr dii={dii_net:+.0f}Cr both={bool(both)})")
    except Exception as e:
        log.warning(f"FIX-STREAK: _append_fii_dii_history failed (non-fatal): {e}")


def _get_fii_streak() -> dict:
    """
    FIX-STREAK: Read last 5 rows from fii_dii_history and return streak metrics.
    Returns streak_bonus (0-8 pts) added to fii_dii score in fetch_fii_dii().
    """
    NEUTRAL_STREAK = {"fii_consec": 0, "dii_consec": 0, "both_consec": 0,
                      "n_rows": 0, "streak_label": "", "streak_bonus": 0}
    try:
        with _db_conn(write=False) as con:
            rows = con.execute(
                "SELECT fii_net_cr, dii_net_cr, both_buying "
                "FROM fii_dii_history ORDER BY trade_date DESC LIMIT 5"
            ).fetchall()
    except Exception as e:
        log.debug(f"FIX-STREAK: _get_fii_streak DB read failed: {e}")
        return NEUTRAL_STREAK
    ...
    fii_c  = _consec(0)
    dii_c  = _consec(1)
    both_c = sum(1 for r in rows if r[2] == 1)

    if fii_c >= 3 and dii_c >= 3:
        bonus, label = 8, f"🔥 ACCUMULATING {fii_c}d streak"
    elif fii_c >= 2 and dii_c >= 2:
        bonus, label = 4, f"📈 Building streak ({fii_c}d)"
    elif both_c >= 3:
        bonus, label = 4, f"✅ {both_c}/5 days both buying"
    else:
        bonus, label = 0, ""

    return {"fii_consec": fii_c, "dii_consec": dii_c, "both_consec": both_c,
            "n_rows": len(rows), "streak_label": label, "streak_bonus": bonus}
...
-- FIX-STREAK: Daily FII/DII history — enables consecutive-session accumulation scoring.
-- Populated by _append_fii_dii_history() called in run() after intelligence fetch.
-- Kept rolling 30 days; streak counter reads last 5 rows.
CREATE TABLE IF NOT EXISTS fii_dii_history (
    trade_date  TEXT PRIMARY KEY,
    fii_net_cr  REAL NOT NULL,
    dii_net_cr  REAL NOT NULL,
    both_buying INTEGER NOT NULL DEFAULT 0,  -- 1 if fii_net_cr>0 AND dii_net_cr>0
    created_at  TEXT DEFAULT (datetime('now'))
);
...
def _select_lane_winner(lane_results, halal_map, alpha_mine_map, lane_name,
    ...
    for r in lane_results:
        ...
        if lane_name == "FORTRESS":
            ...
            # FIX-BUNKER: reject mid-air entries — CMP > buy_hi + 3% tolerance
            if close > 0 and close > r["buy_hi"] * 1.03:
                log.info(f"BUNKER GATE [{lane_name}]: {sym} CMP ₹{close:.0f} > "
                         f"buy_hi ₹{r['buy_hi']:.0f} — tagged CHASING, demoted to WATCHLIST")
                r["data_quality"] = "CHASING_BUNKER_REJECT"
                r["grade"] = "WATCHLIST"
                r["story"] = (r.get("story", "") +
                              f" | ⛔ CHASING: CMP ₹{close:.0f} above entry box ₹{r['buy_hi']:.0f}")
                continue  # skip to next candidate in lane
            ...
            # FIX-BUNKER: reject mid-air entries — CMP > buy_hi + 3% tolerance
            close = r.get("close", 0)
            buy_hi = r.get("buy_hi", 0)
            if close > 0 and buy_hi > 0 and close > buy_hi * 1.03:
                log.info(f"BUNKER GATE [{lane_name}]: {sym} CMP ₹{close:.0f} > "
                         f"buy_hi ₹{buy_hi:.0f} — tagged CHASING, demoted to WATCHLIST")
                r["data_quality"] = "CHASING_BUNKER_REJECT"
                r["grade"] = "WATCHLIST"
                r["story"] = (r.get("story", "") +
                              f" | ⛔ CHASING: CMP ₹{close:.0f} above entry box ₹{buy_hi:.0f}")
                continue  # skip to next candidate in lane
...
def run():
   ...
   10. Weekly agent (--weekly-review) + Sandbox proposals (--sandbox-proposals)
    ...
    # FIX-STREAK: persist today's FII/DII to SQLite history for streak scoring
    _append_fii_dii_history(fii_data, date_label)
